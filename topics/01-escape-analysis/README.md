...
**那多大的差距才算差距？** 本专题留了一对函数体逐字相同的基准当噪声地板
（`NoiseFloor_A` / `NoiseFloor_B`），实测 34.20 ns vs 35.20 ns —— **2.9%**。
小于这个数的差异不该被解释成结论。

注意噪声地板本身也是环境的函数：这个 2.9% 是在安静机器（load 3.4）上量的，
专题 02 在同一台机器较忙时量到的是 **6.4%**。**每批数据都该重新量一次**，
别拿别人的地板套自己的数。
...
**噪声地板**（单独测，机器安静时 load 3.4）：

| 基准 | sec/op | 说明 |
|---|---:|---|
| `NoiseFloor_A` | 34.20n ±2% | 和 `NoiseFloor_B` 函数体逐字相同 |
| `NoiseFloor_B` | 35.20n ±1% | 差 **2.9%** |

上表的主数据和这对噪声地板不是同一批采集的，所以 2.9% 只是量级参考。
要严格比较时，噪声地板应当和主数据同批跑出来。
